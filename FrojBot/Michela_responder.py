...
def best_outlines(spelling, outlines, complexity):
    ...
    if complexity == "summarise all":
        output = f"Showing `{number_of_best_entries}`/`{total_number_of_entries}` outlines for '{spelling}' in Michela theory\n{output}{too_big}"
    else:
        output = f"Here's the best {number_of_best_entries}/{total_number_of_entries} entries for '{spelling}' in Michela theory\n{output}{too_big}"
...
def best_entries(outline, spellings, complexity):
    if not complexity == "annotate best":
        ...
        return f"Found {len(list_of_all_spellings)} matches for `{outline}` in Michela theory (🏴󠁧󠁢󠁥󠁮󠁧󠁿 edition)\n`{outline}` → `{'`/`'.join(list_of_all_spellings)}`"
    ...
    output = f"Here's the {total_number_of_spellings} entries for `{initial_raw_steno}` in Michela theory (best first)\n{output}\n{too_big}"
...
def get_Michela_response(word_to_find: str, complexity: int) -> str:

    if complexity == 1:
        complexity = "summarise best"

    if complexity == 2:
        complexity = "annotate best"

    if complexity == 3:
        complexity = "summarise all"
    ...
    with open("FrojBot/words_that_Edinburgh_has.txt") as file:
        ...
        for line in file:
            if line.strip() == word_to_find:
                if lookup_type == 'entries':
                    return "Sorry, that's not in Michela's dictionary"
                return "Sorry, not in Michela's base (yet?)"
