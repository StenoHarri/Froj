...
def get_lookup_data(letter: str, lookup_type: str) -> Dict[str, dict]:
    ...
    if letter not in _lookup_data_cache[lookup_type]:
        ...
        filename = os.path.join("FrojBot", "theories", "Tadpole", f"{lookup_type}_starting_{letter}.json")
...
def best_entries(outline, spellings, complexity):
    if not complexity == "annotate best":
        ...
        return f"Found {len(list_of_all_spellings)} matches for `{outline}` in Tadpole theory (🏴󠁧󠁢󠁥󠁮󠁧U+E007F edition)\n`{outline}` → `{'`/`'.join(list_of_all_spellings)}`"
...
def get_Tadpole_response(word_to_find: str, complexity: int) -> str:
